...
class Data
{
    ...
    public function __construct(string $path, string $date)
    {
        ...
        $this->price = (new Price($path, $date))->getData();
    ...
    private function readEps()
    {
        ...
        try {
            foreach ($data as $item) {
                ...
                $pushPrice = 0;
                if ($item['19'] != '') {
                    $price = $this->price->get($item[0]);

                    if (isset($price[$item['19']])) {
                        $pushPrice = $price[$item['19']];
                    } else {
                        foreach ($price as $d => $v) {
                            $pushDate = $d;
                            if ($d >= $item['19']) {
                                break;
                            }
