    ...
    protected function insert(Collection $data): bool
    {
        ...
        foreach ($data as $value) {
            ...
            foreach ($p as $i => $v) {
                ...
                $result = true;
                $d = $date[$i];
                $model = $this->priceRepo->get($code, $d);

                if ($model == null) {
                    $result = $this->priceRepo->insert($this->new(collect([
                        'code' => $code,
                        'date' => $d,
                        $this->key() => $v,
                    ])));
                ...
                if (! $result) {
                    throw new StockException($code, 'save error for ' . $d);
                }
            }

            $this->info('code: ' . $code . ' total: ' . count($p) . ' insert: ' . $insertTotal . ' update: ' . $updateTotal);
        ...
        $this->info('total: ' . count($date) * $data->count() . ' insert: ' . $insertAllTotal . ' update: ' . $updateAllTotal);
